...
from saler.models import Product, ProductSize, dow, category, Orders, trend, ProductReview, Blogs
...
def productView(request, prod_id):
	...
	current_product = Product.objects.get(product_id=prod_id)
	
	categories = category.objects.get(name = current_product.category)
	print(categories.sub_Categories)

	products_in_category = Product.objects.filter(category=current_product.category).exclude(subcategory=current_product.subcategory)

	products_in_category = list(products_in_category)

	product_percentage_similarity = {}

	for product in products_in_category:
		current_desc = str(current_product.desc)
		other_desc = str(product.desc)

		current_desc = clean_text(current_desc)
		other_desc = clean_text(other_desc)

		embedding1 = use_model([current_desc])[0]
		embedding2 = use_model([other_desc])[0]
		cosine_sim = round(((cosine_similarity(embedding1, embedding2)) * 100) , 2)

		product_percentage_similarity.update({product: cosine_sim})
	
	print(product_percentage_similarity)
	sorted_products = sorted(product_percentage_similarity.items(), key=lambda x: x[1], reverse=True)
    # Select four unique random products excluding the current product
	selected_products = [item[0] for item in sorted_products[:5]]
	print(selected_products)
	# for product in selected_products:
	global all_visits
	all_visits.append(current_product.category)
	...
	for cat in cats:
		prods= []
		for p in [i for i in Product.objects.filter(category=cat)]:
			prods.append([p,[item for item in ProductSize.objects.filter(product=p)]])
		n = len(prods)
		nSlides = 5
		allProds.append([prods[::-1], range(1, nSlides), nSlides])

	similar_products = Product.objects.filter(category=current_product.category,subcategory = current_product.subcategory)
...
def blogs(request):
	...
	print(blog)
	print(all_blogs)
	current_blog = blog.desc

	for product in all_blogs:
		
		other_desc = str(product.desc)
		other_desc = clean_text(other_desc)

		embedding1 = use_model([current_blog])[0]
		embedding2 = use_model([other_desc])[0]
		cosine_sim = round(((cosine_similarity(embedding1, embedding2)) * 100) , 2)

		blogs.update({product: cosine_sim})
	# Shuffle the order of the blogs
	shuffled_blogs = list(all_blogs)
	...
	params = {
		...
		'trend': trend.objects.order_by('-number')[0:30],
		'cart_element_no' : len([p for p in Cart.objects.all() if p.user == request.user]),
		'blogs' : shuffled_blogs
	}
	return  render(request,"main/blogs.html", params)
...
def show_blog(request, id):
	...
	return  render(request,"main/show_blog.html", params)