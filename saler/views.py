...
from .models import SalerDetail, Product, ProductSize, SellerSlider, MyCart, WholeSaleProduct, category, Orders, WholeSaleProductOrders, Blogs
	...
	if request.user.is_superuser or request.user.is_staff:
		if request.method == 'POST':
			...
			if Blogs.objects.all():
				blog_id2 = 'pr'+hex(Product.objects.all().last().product_id+1)
			else:
				blog_id2 = 'pr'+hex(0)
			image1 = request.FILES.get("image1")
			image2 = request.FILES.get("image2")
			...
			Blogs(blog_id2=blog_id2,shop=request.user,blog_title=blog_title,blog_category=categorys,desc=desc,desc2=desc2,desc3=desc3, image1=image1, image2=image2,image3=image3).save()
			p = Blogs.objects.filter(blog_id2=blog_id2)[0]

			
			p.save()
		blogs = [p for p in Blogs.objects.all() if p.shop == request.user]

		if request.method == 'GET':
			blog_id = request.GET.get('blog_id', 0)
			del_prod = [p.blog_id for p in Blogs.objects.all() if p.shop == request.user]
			if int(blog_id) in del_prod:
				Blogs.objects.filter(blog_id=int(blog_id))[0].delete()
				messages.success(request, f'The Blog of id {blog_id} is deleted !')
				return redirect('/seller/add_blog/')
		...
		return render(request, 'saler/add_blog.html',params)
	else:
		return redirect("/")